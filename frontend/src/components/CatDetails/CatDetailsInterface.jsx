...
export default function CatDetailsInterface() {
  ...
  const handleRelease = async () => {
    ...
    try {
      ...
      alert(t("catDetails.releaseFailed"));
    }
  };
  ...
  if (!cat || error) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center gap-4">
        <p className="opacity-70 text-lg">{t("catDetails.notFound")}</p>
        <button className="btn btn-primary rounded-full px-6" onClick={() => navigate("/catbox")}>
          {t("catDetails.backToCatBox")}
        </button>
      </div>
    );
  }
