...
export default function BorrowCatPanel({ friendshipId, onBorrowed }) {
    ...
    const load = useCallback(async () => {
        ...
        try { setCats(await getFriendCats(friendshipId)); }
    ...
    const handleBorrow = async (cat) => {
        ...
        try {
            ...
            setFeedback({ ok: true, msg: t("friends.borrowing", { name: cat.name }) });
            onBorrowed?.(cat.id);
            load();
        } catch (e) {
    ...
    if (cats.length === 0) return <p className="text-xs opacity-60 pt-1">{t("friends.noCats")}</p>;
        ...
        <div className="space-y-1 pt-1">
            ...
            {cats.map(cat => (
                <div key={cat.id} className="flex items-center gap-2 bg-base-100 rounded-lg px-2 py-1.5">
                    {cat.image && <img src={cat.image} alt={cat.name} className="w-7 h-7 rounded-full object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold truncate">{cat.name}</p>
                        <p className="text-xs opacity-50 truncate">{cat.breed}</p>
                    </div>
                    {cat.available ? (
                        <button type="button" onClick={() => handleBorrow(cat)} disabled={borrowingId !== null} className="btn btn-xs btn-primary shrink-0">
                            {borrowingId === cat.id ? <span className="loading loading-spinner loading-xs" /> : t("friends.borrow")}
                        </button>
                    ) : (
                        <span className="text-xs opacity-50 shrink-0 italic">
                            {cat.borrowedByUsername ? `@${cat.borrowedByUsername}` : t("friends.inUse")}
                        </span>
                    )}
                </div>
            ))}
